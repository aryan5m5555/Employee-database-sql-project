...
VALUES (100,'Steven','King','steven.king@sqltutorial.org','515.123.4567','1987-06-17',4,24000.00,100,9),
...
--1.Count How many employees are currently in the system
select count(EmployeeID) as Total_Emloyees from employees; 


--2.Count employees in each department. (GROUP BY)
select d.departmentname,count(e.employeeID) as totalEmployees from employees e
join departments d
on d.departmentid = e.departmentid 
group by departmentname;


--3.Show each country along with the number of locations in that country.
select countryid,count(*) as NumberOfLocation
from Locations
group by countryid;


--4.List employees along with their job titles.(Join employees → jobs)
Select e.employeeID,
       e.firstName,
       e.lastName,
       j.jobTitle
...
--6.For each region, display total number of countries.
select r.regionname,count(c.countryname) as totalCountries
from regions r
join countries c
...
--7.Which Department have the highest number of employees
select d.departmentname,count(e.employeeID) as totalemployee from employees e 
join departments d 
on d.departmentID = e.departmentID 
group by departmentName
...
--8.What is the average salary per department
 Select d.departmentname,round(avg(e.salary),2) as AverageSalary from departments d
 join employees e
 on d.departmentid = e.departmentid
 group by departmentname
...
--10.what is the total salary expenditure across the company
select sum(salary) as TotalExpenditure from employees;


--11.Show each employee with their dependent name. (INNER JOIN)
Select concat(e.firstname,' ',e.lastName) as Employee_Name,
       concat(d.firstname,' ',d.lastName) as Dependent_Name,d.relationship
from employees e 	   
inner join dependents d
on e.employeeid = d.employeeid;


--12.Show employees who do not have dependents. (LEFT JOIN)
Select concat(e.firstname,' ',e.lastName) as Employee_Name,
       e.employeeid     
from Employees e
...
--13.Find employees with the same manager (self join).
select m.firstname as EmployeeName,
       ee.firstName as ManagerName
from employees m
join employees ee
on m.ManagerID = ee.employeeid;


--14.Find employees earning above the average salary in their department.
select e.employeeid,e.salary,concat(e.firstname,' ',e.lastname) as name
from employees e
where salary > (
select avg(salary) from employees ee
where e.departmentid = ee.departmentid)

--using join
 Select e.departmentid,e.salary,e.firstname
 from employees e
 join (select departmentid,avg(salary) as avgsalary
 from employees 
  group by departmentid ) as ee
 on e.departmentid = ee.departmentid and e.salary > ee.avgsalary ;
