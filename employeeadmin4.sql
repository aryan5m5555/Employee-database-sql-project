...
-- Count How many employees are currently in the system
...
--Show each country along with the number of locations in that country.
select countryid,count(*) as NumberOfLocation
from Locations
group by countryid;


--List employees along with their job titles.(Join employees → jobs)
Select e.employeeID,
       e.firstName,
       e.lastName,
       j.jobTitle
...
--For each region, display total number of countries.
...
--Show each employee with their dependent name. (INNER JOIN)
...
--Show employees who do not have dependents. (LEFT JOIN)
Select concat(e.firstname,' ',e.lastName) as Employee_Name,
       e.employeeid     
from Employees e
...
-- Find employees with the same manager (self join).
